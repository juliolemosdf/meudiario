  ...
  for (const msg of messages) {
    if (msg.comparativeId && comparativePairs[msg.comparativeId]) {
      ...
      try {
        ...
        console.error('Erro ao ler imagens comparativas:', error);
      }
...
export default function App() {
  ...
  const saveUserSetup = async () => {
    ...
    if (!tempAvatar) {
      Alert.alert('Erro', 'Por favor, tire uma foto de perfil.');
      return;
    }
  ...
  const filterMessages = (messages, type) => {
    ...
    return messages.filter((message) => {
      if (type === 'texto' && message.type === 'text' && message.text) return true;
      if (type === 'imagem' && (message.type === 'single' || message.type === 'comparative')) return true;
      if (type === 'vídeo' && message.type === 'video') return true;
      if (type === 'áudio' && message.type === 'audio') return true;
      return false;
    });
  };
  ...
  const handleGenerateReport = async (reportType) => {
    ...
    switch (reportType) {
      case 'texto':
        ...
        break;
      case 'imagem':
        filteredMessages = messages.filter((msg) => msg.type === 'single' || msg.type === 'comparative');
        break;
      case 'todos':
            ...
            <View style={styles.modalOverlay}>
              <ScrollView contentContainerStyle={styles.userSetupContainer}>
                ...
                >
                  ...
                  <Text style={styles.avatarPickerButtonText}>Tirar Foto de Perfil</Text>
                </TouchableOpacity>
          ...
          >
            <View style={styles.modalOverlay}>
              <View style={styles.imageOptionsContainer}>
                <Text style={styles.imageOptionsTitle}>Escolha a Opção</Text>
                <TouchableOpacity style={styles.imageOptionButton} onPress={handleTakePhoto}>
                ...
                <TouchableOpacity style={styles.imageOptionButton} onPress={handleSingleImagePick}>
                  <Text style={styles.imageOptionText}>Escolher Foto da Galeria</Text>
                </TouchableOpacity>
                <TouchableOpacity
